...
class CollectorTask(object):
    ...
    def _refresh_dataset(self):
        ...
        self.logger.debug("[COLLECTOR]: refreshing dataset.")
    ...
    def base_dataset(self):
        ...
        if os.path.exists(self._backup_dir):
            self.logger.debug("[COLLECTOR]:list the backup directory {}".format(self._backup_dir))
        ...
        self.logger.debug("[COLLECTOR]:fetch base dataset succ.")
    ...
    def update_container(self, handle):
        ...
        if handle not in self._base_dataset:
            self.logger.debug("[COLLECTOR]: {} not in dataset, unregister it.".format(handle))
    ...
    def unregister_containers_from_collector(self, handle):
        ...
        self.logger.debug("[COLLECTOR]: container {} state change : {} -> {}".format(handle, 'N/A', local_state))
    ...
    def update_container_state(self, handle):
        ...
        self.logger.debug('[COLLECTOR]: query state of {}.'.format(handle))
        if error:
            self.logger.debug('[COLLECTOR]: query state of {} with error {}.'.format(handle, error))
            return
        ...
        if not collector_state:
            if not local_state:
                self.logger.debug('[COLLECTOR]: container {} not recorded in local and collector.'.format(handle))
                return
            self.request_collector('state_update', ip=local_ip(), handle=handle, state=local_state)
            self.logger.debug("[COLLECTOR]: container {} state change : {} -> {}".format(handle, 'N/A', local_state))
            ...
            local_state = 'STOPPED'
            self.request_collector('state_update', ip=local_ip(), handle=handle, state=local_state)
            self.logger.debug("[COLLECTOR]: container {} state change : {} -> {}".format(handle, collector_state, local_state))
        else:
            pass
    ...
    def push_container_list(self):
        ...
        """
        self.logger.debug("[COLLECTOR]: push container list to collector.")
        local = local_ip()
        resp, error = self.request_collector('all_containers', ip=local, containers=list(self._base_dataset))
        if error:
           self.logger.debug("[COLLECTOR]: push container list to collector failed with {}".format(error))
    ...
    def start(self, notified_dir, event):
        ...
        if len(notify_check) > 0:
            self.logger.debug("[COLLECTOR]: ignore etcd task trigger event.")
        elif notified_dir == 'snapshot-changed':
            if event == 'create':
                self._refresh_dataset()
                self.refresh_inactive_containers()
                self.logger.debug("[COLLECTOR]: refresh inactive containers.")
            else:
